...
//
// Range
func Range[T constraints.Float | constraints.Integer](value T, params ...T) bool {
	if len(params) == 2 {
		// value, _ := strconv.ParseFloat(str, 64)
		// min,max:= float64(params[0]),float64(params[1])
		return InRange(value, params[0], params[1])
	}
...
// RuneLength checks string's length
// Alias for StringLength
func RuneLength[T constraints.Float | constraints.Integer](str string, params ...T) bool {
	return StringLength(str, params...)
}

// StringLength checks string's length (including multi byte strings)
func StringLength[T constraints.Float | constraints.Integer](str string, params ...T) bool {
	if len(params) == 2 {
		strLength := T(utf8.RuneCountInString(str))
		return strLength >= params[0] && strLength <= params[1]
	}
