...
func (mysql) GetTables(db *DB, dbName string) (map[string][]dialect.Column, error) {
	rows, err := db.Query(`SELECT table_name,column_name,column_default,data_type,character_maximum_length,column_key,extra,column_comment FROM information_schema.COLUMNS WHERE table_schema = ?`, dbName)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
