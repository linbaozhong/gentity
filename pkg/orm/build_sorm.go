...
import (
	...
	"github.com/linbaozhong/gentity/pkgs/conv"
...
type ISqlBuilder interface {
	...
	Select(dest interface{}, x ...*sqlx.Tx) (bool, error)
	Get(dest interface{}, x ...*sqlx.Tx) (bool, error)
	// Queryx(x ...*sqlx.Tx) (*sqlx.Rows, error)
	Insert(x ...*sqlx.Tx) (sql.Result, error)
	Update(x ...*sqlx.Tx) (sql.Result, error)
	Delete(x ...*sqlx.Tx) (sql.Result, error)
	...
	// JOIN
	Join(t JoinType, l, r TableField, args ...ISqlBuilder) ISqlBuilder
	GetJoin() [][3]string
	// LIMIT
	...
	GetExpr() []Expr
	//
	Max(f TableField) ISqlBuilder
	Min(f TableField) ISqlBuilder
	Sum(f TableField, as ...string) ISqlBuilder
	Count(as ...string) ISqlBuilder
...
// and
func And(sb ISqlBuilder) ISqlBuilder {
	return NewSqlBuilder().And(sb)
}
...
func (p *sqlBuilder) exec(query string, args []interface{}, x ...*sqlx.Tx) (result sql.Result, e error) {
	if len(x) == 0 || x[0] == nil {
		result, e = Db().Exec(query, args...)
	} else {
		result, e = x[0].Exec(query, args...)
	}
	log.Info(query, args)
	return
}
...
//
// //Insert
// func (p *sqlBuilder) Insert() (query string, args []interface{}, e error) {
//	defer p.Free()
//	query, args, e = p.getInsert()
//	return query, args, e
//
//	//
...
// Insert 如果有自增id，返回自增id
func (p *sqlBuilder) Insert(x ...*sqlx.Tx) (result sql.Result, e error) {
	defer p.Free()
	query, args, e := p.getInsert()
	if e != nil {
		return
	...
	return p.exec(query, args, x...)
}

// InsertStruct
func (p *sqlBuilder) InsertStruct(bean interface{}, x ...sqlx.Tx) (i64 int64, e error) {
	v := reflect.ValueOf(bean)
	if v.Kind() != reflect.Ptr {
		return 0, errors.New("bean 必须是一个指针，而不是一个值")
	}
	if v.IsNil() {
		return 0, errors.New("bean 不能是nil指针")
	}
	...
	case reflect.Slice:
	default:
		return 0, errors.New("bean 必须是一个struct或者slice指针")
	}
...
// Update
func (p *sqlBuilder) Update(x ...*sqlx.Tx) (result sql.Result, e error) {
	defer p.Free()
	query, args, e := p.getUpdate()
	if e != nil {
		return
	}
...
// Delete
func (p *sqlBuilder) Delete(x ...*sqlx.Tx) (result sql.Result, e error) {
	defer p.Free()
	query, args, e := p.getDelete()
	if e != nil {
		return
	}
	return p.exec(query, args, x...)
...
// Select
func (p *sqlBuilder) Select(dest interface{}, x ...*sqlx.Tx) (has bool, e error) {
	defer p.Free()

	query, args, e := p.getSelect()
	log.Info(query, args)
	if e != nil {
		return false, e
	}
	...
	if len(x) == 0 || x[0] == nil {
		e = Db().Select(dest, query, args...)
	} else {
		e = x[0].Select(dest, query, args...)
	}
...
// Get
func (p *sqlBuilder) Get(dest interface{}, x ...*sqlx.Tx) (has bool, e error) {
	defer p.Free()
	...
	if len(x) == 0 || x[0] == nil {
		e = Db().Get(dest, query, args...)
	} else {
		e = x[0].Get(dest, query, args...)
	}
	if e == nil {
		return true, nil
	}
...
// //todo: Queryx
// func (p *sqlBuilder) Queryx(x ...*sqlx.Tx) (*sqlx.Rows, error) {
//	defer p.Free()
//
//	query, args, e := p.getSelect()
//	if e != nil {
...
// JOIN
func (p *sqlBuilder) Join(t JoinType, l, r TableField, args ...ISqlBuilder) ISqlBuilder {
	var where string
	if len(args) > 0 {
		where = " AND ( " + args[0].GetWhereString() + " )"
		params := args[0].GetParams()
		p.whereParams = append(p.whereParams, params...)
	}
	p.join = append(p.join, [3]string{
		string(t),
		Quote_Char + r.Table + Quote_Char,
		l.Quote() + " = " + r.Quote() + where,
	})
	return p
}
...
func (p *sqlBuilder) Max(f TableField) ISqlBuilder {
	p.cols = append(p.cols, "Max("+f.Quote()+")")
	return p
}

func (p *sqlBuilder) Min(f TableField) ISqlBuilder {
	p.cols = append(p.cols, "Min("+f.Quote()+")")
	return p
}

func (p *sqlBuilder) Sum(f TableField, as ...string) ISqlBuilder {
	var a = f.Name
	if len(as) > 0 {
		a = as[0]
	}
	p.cols = append(p.cols, "IFNULL(Sum("+f.Quote()+"),0) AS "+a)
	return p
}

func (p *sqlBuilder) Count(as ...string) ISqlBuilder {
	var a = "count"
	if len(as) > 0 {
		a = as[0]
	}
	p.cols = append(p.cols, "COUNT(1) AS "+a)
	return p
}
...
func (p *sqlBuilder) String() string {
	...
	for _, i := range pm {
		buf.WriteString(conv.Interface2String(i) + "|")
	}
	return buf.String()
}
...
func (p *sqlBuilder) condition() (string, []interface{}) {
	...
	// WHERE
	if p.where.Len() > 0 {
		buf.WriteString(" WHERE " + p.GetWhereString())
	}
	// GROUP BY
	if p.groupBy.Len() > 0 {
...
func (p *sqlBuilder) getInsert() (string, []interface{}, error) {
	...
	buf.WriteString(" )")
	// buf.WriteString(";SELECT LAST_INSERT_ID();")
...
func (p *sqlBuilder) getUpdate() (string, []interface{}, error) {
	...
	for _, col := range p.exprCols {
		cols = append(cols, col.ColName+" = "+conv.Interface2String(col.Arg))
	}
