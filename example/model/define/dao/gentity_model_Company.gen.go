...
// UpdateById
func (p *daoCompany) UpdateById(ctx context.Context, id uint64, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
...
// DeleteById
func (p *daoCompany) DeleteById(ctx context.Context, id uint64) (bool, error) {
	return p.Delete(ctx,
		tblcompany.PrimaryKey.Eq(id),
	)
}
...
func (p *daoCompany) Get4Cols(ctx context.Context, cols []dialect.Field, cond ...dialect.Condition) (*db.Company, bool, error) {
	c := p.R()
	if len(cols) == 0 {
...
func (p *daoCompany) Find4Cols(ctx context.Context, pageIndex, pageSize uint, cols []dialect.Field, cond ...dialect.Condition) ([]*db.Company, bool, error) {
	c := p.R()
	if len(cols) == 0 {
		c.Cols(tblcompany.ReadableFields...)
	} else {
...
func (p *daoCompany) GetByID(ctx context.Context, id uint64, cols ...dialect.Field) (*db.Company, bool, error) {
	return p.Get4Cols(ctx, cols, tblcompany.PrimaryKey.Eq(id))
}

// Get 按条件读取一个company对象,先判断第二返回值是否为true,再判断是否第三返回值为nil
func (p *daoCompany) Get(ctx context.Context, cond ...dialect.Condition) (*db.Company, bool, error) {
	return p.Get4Cols(ctx, []dialect.Field{}, cond...)
}

// GetFirstCell 按条件读取首行首列,先判断第二返回值是否为true,再判断是否第三返回值为nil
func (p *daoCompany) GetFirstCell(ctx context.Context, col dialect.Field, cond ...dialect.Condition) (any, bool, error) {
	c := p.R().Cols(col)
...
func (p *daoCompany) Find(ctx context.Context, pageIndex, pageSize uint, cond ...dialect.Condition) ([]*db.Company, bool, error) {
	return p.Find4Cols(ctx, pageIndex, pageSize, []dialect.Field{}, cond...)
}

// IDs
func (p *daoCompany) IDs(ctx context.Context, cond ...dialect.Condition) ([]any, error) {
...
// Columns
func (p *daoCompany) Columns(ctx context.Context, col dialect.Field, cond ...dialect.Condition) ([]any, error) {
	c := p.R().Cols(col)
	rows, err := c.Where(cond...).
		Limit(dialect.MaxLimit).
		Query(ctx)
