...
type UserDaoer interface {
	...
	// cols: 要插入的列名
	InsertMulti(ctx context.Context, beans []*db.User, cols ...dialect.Field) (int64, error)
	// UpdateMulti 批量更新多条数据
	// cols: 要更新的列名
	UpdateMulti(ctx context.Context, beans []*db.User, cols ...dialect.Field) (bool, error)
	...
	GetByID(ctx context.Context, args ...any) (*db.User, bool, error)
	// Get 按条件读取一个对象
...
// cols: 要插入的列名
func (p *userDao) InsertMulti(ctx context.Context, beans []*db.User, cols ...dialect.Field) (int64, error) {
	lens := len(beans)
	if lens == 0 {
...
// UpdateMulti
// cols: 要更新的列名
func (p *userDao) UpdateMulti(ctx context.Context, beans []*db.User, cols ...dialect.Field) (bool, error) {
	lens := len(beans)
	if lens == 0 {
...
func (p *userDao) GetByID(ctx context.Context, args ...any) (*db.User, bool, error) {
	lens := len(usertbl.PrimaryKeys)
	if lens != len(args) {
		return nil, false, atype.ErrArgsNotMatch
	}

	cond := make([]dialect.Condition, 0, lens)
	for i, key := range usertbl.PrimaryKeys {
...
func (p *userDao) GetSingle(ctx context.Context, col dialect.Field, cond ...dialect.Condition) (any, error) {
	c := p.R().Cols(col)
...
// IDs
func (p *userDao) IDs(ctx context.Context, cond ...dialect.Condition) ([]int64, error) {
	if len(usertbl.PrimaryKeys) == 0 {
		return nil, atype.ErrPrimaryKeyNotMatch
	}
	c := p.R().Cols(usertbl.PrimaryKeys[0])
	rows, err := c.Where(cond...).Limit(atype.MaxLimit).Query(ctx)
	if err != nil {
		return nil, err
	}
...
func (p *userDao) Exists(ctx context.Context, cond ...dialect.Condition) (bool, error) {
	...
	c := p.R().Cols(usertbl.PrimaryKeys[0]).Where(cond...).Limit(1)
