...
type company_maner interface {
	...
	InsertBatch(ctx context.Context, beans []*db.CompanyMan, cols ...dialect.Field) (int64, error)
	...
	UpdateBatch(ctx context.Context, beans []*db.CompanyMan, cols ...dialect.Field) (bool, error)
	...
	// Find 分页查询，返回一个slice
	Find(ctx context.Context, pageIndex, pageSize uint, cond []dialect.Condition, sort ...dialect.Order) ([]db.CompanyMan, bool, error)
	// Get4Cols 读取一个对象的指定列
	Get4Cols(ctx context.Context, cols []dialect.Field, cond []dialect.Condition, sort ...dialect.Order) (*db.CompanyMan, bool, error)
	// GetByID 按主键查询，返回一个对象
	GetByID(ctx context.Context, id types.BigInt, cols ...dialect.Field) (*db.CompanyMan, bool, error)
	// Get 按条件读取一个对象
	Get(ctx context.Context, cond []dialect.Condition, sort ...dialect.Order) (*db.CompanyMan, bool, error)
...
type daoCompanyMan struct {
	db ace.Executer
}

func CompanyMan(exec ...ace.Executer) company_maner {
	_obj := &daoCompanyMan{}
...
func CompanyManCreate(exec ...ace.Executer) ace.CreateBuilder {
	return Create(exec...).Table(db.CompanyManTableName)
}

// Select company_man
func CompanyManSelect(exec ...ace.Executer) ace.SelectBuilder {
	return Select(exec...).Table(db.CompanyManTableName)
}

// Update company_man
func CompanyManUpdate(exec ...ace.Executer) ace.UpdateBuilder {
	return Update(exec...).Table(db.CompanyManTableName)
}

// Delete company_man
func CompanyManDelete(exec ...ace.Executer) ace.DeleteBuilder {
	return Delete(exec...).Table(db.CompanyManTableName)
}

// Insert 返回 LastInsertId
func (p *daoCompanyMan) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	if len(sets) == 0 {
...
// InsertBatch 批量插入,返回 RowsAffected。禁止在事务中使用
// cols: 要插入的列名，如果为空，则插入结构体字段对应所有列
func (p *daoCompanyMan) InsertBatch(ctx context.Context, beans []*db.CompanyMan, cols ...dialect.Field) (int64, error) {
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *daoCompanyMan) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	if len(sets) == 0 {
...
// UpdateById
func (p *daoCompanyMan) UpdateById(ctx context.Context, id types.BigInt, sets ...dialect.Setter) (bool, error) {
	return p.Update(ctx,
		sets,
...
// UpdateBatch 批量更新,禁止在事务中使用
// cols: 要更新的列名，如果为空，则更新结构体所有字段对应列，包含零值字段
func (p *daoCompanyMan) UpdateBatch(ctx context.Context, beans []*db.CompanyMan, cols ...dialect.Field) (bool, error) {
	_lens := len(beans)
	if _lens == 0 {
...
// DeleteById
func (p *daoCompanyMan) DeleteById(ctx context.Context, id types.BigInt) (bool, error) {
	return p.Delete(ctx,
		tblcompanyman.PrimaryKey.Eq(id),
	)
}
...
func (p *daoCompanyMan) GetByID(ctx context.Context, id types.BigInt, cols ...dialect.Field) (*db.CompanyMan, bool, error) {
	return p.Get4Cols(ctx, cols, []dialect.Condition{tblcompanyman.PrimaryKey.Eq(id)})
}

// Get 按条件读取一个company_man对象,先判断第二返回值是否为true,再判断是否第三返回值为nil
func (p *daoCompanyMan) Get(ctx context.Context, cond []dialect.Condition, sort ...dialect.Order) (*db.CompanyMan, bool, error) {
...
// Find 按条件读取一个company_man slice对象,先判断第二返回值是否为true,再判断是否第三返回值为nil
func (p *daoCompanyMan) Find(ctx context.Context, pageIndex, pageSize uint, cond []dialect.Condition, sort ...dialect.Order) ([]db.CompanyMan, bool, error) {
