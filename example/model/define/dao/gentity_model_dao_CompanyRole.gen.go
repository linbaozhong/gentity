...
//	// SelectAll 读取所有数据
//	SelectAll(ctx context.Context, s ace.SelectBuilder) ([]db.CompanyRole, bool, error)
...
func CompanyRole(exec ...ace.Executer) *daoCompanyRole {
	_obj := &daoCompanyRole{}
...
func (p *daoCompanyRole) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	...
	_result, e := CompanyRoleCreate(p.db).
		Sets(sets...).
		Exec(ctx)
	if e != nil {
		log.Error(e)
...
func (p *daoCompanyRole) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	...
	_result, e := CompanyRoleUpdate(p.db).
		Where(cond...).
		Sets(sets...).
		Exec(ctx)
	if e != nil {
...
func (p *daoCompanyRole) Delete(ctx context.Context, cond ...dialect.Condition) (bool, error) {
	_result, e := CompanyRoleDelete(p.db).
		Where(cond...).
		Exec(ctx)
	if e != nil {
		log.Error(e)
...
// SelectAll 查询所有
func (p *daoCompanyRole) SelectAll(ctx context.Context, s ace.SelectBuilder) ([]db.CompanyRole, bool, error) {
	if len(s.GetTableName()) == 0 {
		s.Table(db.CompanyRoleTableName)
	}
...
func (p *daoCompanyRole) GetFirstCell(ctx context.Context, col dialect.Field, cond []dialect.Condition, sort ...dialect.Order) (any, bool, error) {
	_c := CompanyRoleSelect(p.db).Cols(col)
	_row, e := _c.Where(cond...).
		OrderFunc(sort...).
		QueryRow(ctx)
	if e != nil {
...
func (p *daoCompanyRole) IDs(ctx context.Context, cond []dialect.Condition, sort ...dialect.Order) ([]any, error) {
	_c := CompanyRoleSelect(p.db).Cols(tblcompanyrole.PrimaryKey)
	_rows, e := _c.Where(cond...).
		OrderFunc(sort...).
