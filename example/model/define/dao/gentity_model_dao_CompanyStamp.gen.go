...
// ToSql 仅打印SQL语句，不执行
func (p *companystamp) ToSql() *companystamp {
	p.toSql = true
	return p
}

// Insert 返回 LastInsertId
func (p *companystamp) Insert(ctx context.Context, sets ...dialect.Setter) (int64, error) {
	defer p.Free()
	if len(sets) == 0 {
		return 0, dialect.ErrSetterEmpty
	}
	_result, e := ace.Table(do.CompanyStampTableName).
		Set(sets...).
		ToSql(p.toSql).
		Create(p.db).
		Exec(ctx)
...
// InsertOne
// cols: 要插入的列名
func (p *companystamp) InsertOne(ctx context.Context, bean *do.CompanyStamp, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_result, e := ace.Table(do.CompanyStampTableName).
		Cols(cols...).
		ToSql(p.toSql).
...
func (p *companystamp) InsertBatch(ctx context.Context, beans []*do.CompanyStamp, cols ...dialect.Field) (int64, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Update
func (p *companystamp) Update(ctx context.Context, sets []dialect.Setter, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	if len(sets) == 0 {
...
// UpdateOne
// cols: 要插入的列名
func (p *companystamp) UpdateOne(ctx context.Context, bean *do.CompanyStamp, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_result, e := ace.Table(do.CompanyStampTableName).
		Cols(cols...).
		ToSql(p.toSql).
...
func (p *companystamp) UpdateBatch(ctx context.Context, beans []*do.CompanyStamp, cols ...dialect.Field) (bool, error) {
	defer p.Free()
	_lens := len(beans)
	if _lens == 0 {
...
// Delete
func (p *companystamp) Delete(ctx context.Context, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	_result, e := ace.Table(do.CompanyStampTableName).
		Where(cond...).
		ToSql(p.toSql).
...
//
// 返回值:
//  1. *do.CompanyStamp: 第一个符合条件的记录
//  2. bool: 是否存在记录
//  3. error: 错误信息
//
// 注意:
//  1. 如果没有指定表名，则默认使用do.CompanyStampTableName
//  2. 如果没有指定查询列，则默认使用tblcompanystamp.ReadableFields
...
func (p *companystamp) Get(ctx context.Context, s ace.SelectBuilder) (*do.CompanyStamp, bool, error) {
	defer p.Free()
	if len(s.GetTableName()) == 0 {
		s.Table(do.CompanyStampTableName)
	}
	_cols := s.GetCols()
	if len(_cols) == 0 {
...
// 注意:
//  1. 如果没有指定表名，则默认使用do.CompanyStampTableName
//  2. 如果没有指定查询列，则默认使用tblcompanystamp.PrimaryKey
func (p *companystamp) Cell(ctx context.Context, s ace.SelectBuilder) (any, bool, error) {
	defer p.Free()
	if len(s.GetTableName()) == 0 {
		s.Table(do.CompanyStampTableName)
	}
	...
	if len(s.GetCols()) == 0 {
		s.Cols(tblcompanystamp.PrimaryKey)
	}
	_row, e := s.
		ToSql(p.toSql).
		Select(p.db).
...
// List 查询所有符合条件的记录
func (p *companystamp) List(ctx context.Context, s ace.SelectBuilder) ([]*do.CompanyStamp, bool, error) {
	defer p.Free()
	if len(s.GetTableName()) == 0 {
		s.Table(do.CompanyStampTableName)
	}
...
// Count
func (p *companystamp) Count(ctx context.Context, cond ...dialect.Condition) (int64, error) {
	defer p.Free()
	return ace.Table(do.CompanyStampTableName).
		ToSql(p.toSql).
		Select(p.db).
		Count(ctx, cond...)
}
...
// Sum
func (p *companystamp) Sum(ctx context.Context, cols []dialect.Field, cond ...dialect.Condition) (map[string]any, error) {
	defer p.Free()
	return ace.Table(do.CompanyStampTableName).
		ToSql(p.toSql).
		Select(p.db).
...
// Exists
func (p *companystamp) Exists(ctx context.Context, cond ...dialect.Condition) (bool, error) {
	defer p.Free()
	_c := ace.Table(do.CompanyStampTableName).Cols(tblcompanystamp.PrimaryKey).Where(cond...)
	_row, e := _c.
		ToSql(p.toSql).
		Select(p.db).
		QueryRow(ctx)
